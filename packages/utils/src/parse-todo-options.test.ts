...
import { parseDueDateOptions } from "./parse-todo-options";
...
suite("parseTodoOptions", () => {
  suite("check single due date", () => {
    test("no time", () => {
      ...
      const result = parseDueDateOptions(["due:2024-01-01"]);

      assert(isOk(result));
      const dueDate = unwrap(result);
      assert.deepEqual(dueDate?.next, Temporal.PlainDate.from(dateString));
    });

    test("check bad date errors", () => {
      for (const dateString of ["invalid", "2024-99-01"]) {
        const result = parseDueDateOptions(["due:" + dateString]);
  ...
  suite("check daily repeat", () => {
    test("check valid", () => {
      const dateString = "2024-01-01";
      const result = parseDueDateOptions([
        "next:" + dateString,
        "repeat:daily",
      ]);
      assert(isOk(result));
      const dueDate = unwrap(result);
      assert.deepEqual(dueDate?.repeat, "daily");
      assert.deepEqual(dueDate?.next, Temporal.PlainDate.from(dateString));
    });
    ...
    test("every 2 days", () => {
      ...
      assert.deepEqual(dueDate?.repeat, "daily");
      assert.deepEqual(dueDate?.next, Temporal.PlainDate.from(dateString));
      assert.deepEqual(dueDate?.every, 2);
    });
  });
  ...
  suite("check weekly repeat", () => {
    test("check valid", () => {
      ...
      assert(isOk(result));
      const dueDate = unwrap(result);
      assert.deepEqual(dueDate?.repeat, "weekly");
      assert.deepEqual(dueDate?.next, Temporal.PlainDate.from(dateString));
    });
    ...
    test("on day of week", () => {
      ...
      assert.deepEqual(dueDate?.repeat, "weekly");
      assert.deepEqual(dueDate?.next, Temporal.PlainDate.from(dateString));
      assert.deepEqual(dueDate?.dayOfWeek, 1);
    });
  });
  ...
  suite("check monthly repeat", () => {
    test("check valid", () => {
      ...
      assert(isOk(result));
      const dueDate = unwrap(result);
      assert.deepEqual(dueDate?.repeat, "monthly");
      assert.deepEqual(dueDate?.next, Temporal.PlainDate.from(dateString));
    });
    ...
    test("on day of month", () => {
      ...
      assert.deepEqual(dueDate?.repeat, "monthly");
      assert.deepEqual(dueDate?.next, Temporal.PlainDate.from(dateString));
      assert.deepEqual(dueDate?.dayOfMonth, 15);
    });
  ...
  suite("check yearly repeat", () => {
    test("check valid", () => {
      ...
      assert(isOk(result));
      const dueDate = unwrap(result);
      assert.deepEqual(dueDate?.repeat, "yearly");
      assert.deepEqual(dueDate?.next, Temporal.PlainDate.from(dateString));
    });
    ...
    test("on day of year", () => {
      ...
      assert.deepEqual(dueDate?.repeat, "yearly");
      assert.deepEqual(dueDate?.next, Temporal.PlainDate.from(dateString));
      assert.deepEqual(dueDate?.dayOfYear, 15);
    });
